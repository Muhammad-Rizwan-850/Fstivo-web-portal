-- =====================================================
-- PHASE 2: USER EXPERIENCE ENHANCEMENTS - DATABASE SCHEMA
-- =====================================================
...
CREATE TABLE IF NOT EXISTS event_posts (
  ...
  poll_options JSONB, -- {options: [{id, text, votes}], ends_at: timestamp}
...
CREATE TABLE IF NOT EXISTS referrals (
  ...
  reward_type VARCHAR(50), -- 'discount', 'credit', 'free_ticket'
...
CREATE TABLE IF NOT EXISTS social_shares (
  ...
  event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,
  platform VARCHAR(50) NOT NULL, -- 'facebook', 'twitter', 'whatsapp', 'instagram', 'linkedin'
...
CREATE TABLE IF NOT EXISTS pricing_rules (
  ...
  ends_at TIMESTAMPTZ,
  min_quantity INT, -- For group discounts
  max_quantity INT,
  inventory_threshold INT, -- Trigger when X tickets left
  demand_multiplier NUMERIC(5, 2), -- Price increase based on demand
...
CREATE TABLE IF NOT EXISTS ticket_bundles (
  ...
  individual_price NUMERIC(10, 2) NOT NULL, -- Sum of individual tickets
...
CREATE TABLE IF NOT EXISTS season_passes (
  ...
  event_category VARCHAR(100), -- Which events are included
...
CREATE TABLE IF NOT EXISTS checkin_stations (
  ...
  config JSONB, -- Station-specific settings
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS checkin_records (
  ...
  device_info JSONB, -- Browser, OS, location
  checked_in_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  check_out_at TIMESTAMPTZ -- For events that track exit
);
...
CREATE TABLE IF NOT EXISTS badge_print_queue (
  ...
  print_data JSONB NOT NULL, -- Name, photo, QR, custom fields
...
CREATE TABLE IF NOT EXISTS express_lanes (
  ...
  eligibility_criteria JSONB NOT NULL, -- VIP, early bird, specific ticket types
  station_id UUID REFERENCES checkin_stations(id) ON DELETE SET NULL,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS offline_ticket_cache (
  ...
  ticket_id UUID NOT NULL REFERENCES tickets(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  ticket_data JSONB NOT NULL, -- Full ticket details for offline access
...
DROP TRIGGER IF EXISTS trg_update_waitlist_positions ON waitlist;
...
CREATE POLICY "Anyone can view approved photos" ON event_photos
...
CREATE POLICY "Organizers can view pricing rules" ON pricing_rules
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM events
      WHERE id = pricing_rules.event_id AND organizer_id = auth.uid()
    )
  );

CREATE POLICY "Organizers can create pricing rules" ON pricing_rules
...
-- Waitlist policies
CREATE POLICY "Users can view their waitlist" ON waitlist
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can join waitlist" ON waitlist
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Ticket bundles policies
CREATE POLICY "Anyone can view bundles" ON ticket_bundles
  FOR SELECT USING (true);

CREATE POLICY "Organizers can manage bundles" ON ticket_bundles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM events
      WHERE id = ticket_bundles.event_id AND organizer_id = auth.uid()
    )
  );

-- Group bookings policies
CREATE POLICY "Users can view group bookings" ON group_bookings
  FOR SELECT USING (auth.uid() = organizer_user_id);

CREATE POLICY "Users can create groups" ON group_bookings
  FOR INSERT WITH CHECK (auth.uid() = organizer_user_id);

-- Season passes policies
CREATE POLICY "Anyone can view season passes" ON season_passes
  FOR SELECT USING (true);

CREATE POLICY "Organizers can manage passes" ON season_passes
  FOR ALL USING (auth.uid() = organizer_id);

-- Check-in stations policies
CREATE POLICY "Organizers can manage stations" ON checkin_stations
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM events
      WHERE id = checkin_stations.event_id AND organizer_id = auth.uid()
    )
  );

CREATE POLICY "Staff can view assigned stations" ON checkin_stations
  FOR SELECT USING (auth.uid() = assigned_staff);

-- Check-in records policies
CREATE POLICY "Organizers can view checkin records" ON checkin_records
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM checkin_stations
      WHERE id = checkin_records.station_id
        AND EXISTS (
          SELECT 1 FROM events
          WHERE id = checkin_stations.event_id AND organizer_id = auth.uid()
        )
...
CREATE POLICY "Staff can create checkin records" ON checkin_records
  FOR INSERT WITH CHECK (auth.uid() = checked_in_by);
...
CREATE POLICY "Users can manage own subscriptions" ON pwa_push_subscriptions
  FOR ALL USING (auth.uid() = user_id);

-- Offline cache policies
CREATE POLICY "Users can manage own cache" ON offline_ticket_cache
  FOR ALL USING (auth.uid() = user_id);

-- PWA installs policies
CREATE POLICY "Anyone can create install record" ON pwa_installs
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Users can view their installs" ON pwa_installs
  FOR SELECT USING (auth.uid() = user_id);
