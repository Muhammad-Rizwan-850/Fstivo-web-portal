...
-- Enhance existing connections table with connection types (if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'connections') THEN
        ALTER TABLE connections ADD COLUMN IF NOT EXISTS connection_type VARCHAR(20) DEFAULT 'friend'
